...
@implementation LYSChartHistogram
...
- (void)drawHistogram
{
    ...
    for (int i = 0; i < [_valueData count]; i++) {
        ...
        CGPoint point = POINT(self.canvasLeftBottomPoint.x + ((i + 1) * self.columnSpace) - (self.columnSpace * 0.5), self.canvasLeftBottomPoint.y + 0.5);
        
        LYSAnimationLayer *histogramLayer = [self histogramWith:point width:_histogramWidth height:TWOPOINTSPACING(self.canvasLeftTopPoint, self.canvasLeftBottomPoint)];
        ...
        [_chartLayerAry addObject:histogramLayer];
...
#pragma mark - 创建柱状图 -
- (LYSAnimationLayer *)histogramWith:(CGPoint)point width:(CGFloat)width height:(CGFloat)height{
